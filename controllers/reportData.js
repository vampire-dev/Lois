...
Controller.prototype.getPaid = function (query) {
    ...
    return model.find(parameters).sort({ "number": -1 }).populate('sender').populate('payment.type').skip(skip).limit(limit).exec();
};
...
Controller.prototype.getReturns = function (query) {
    ...
    return model.find(parameters).sort({ "number": -1 }).populate('sender destination').skip(skip).limit(limit).exec();
};
    ...
    return co(function* () {
        yield* _co.coEach(viewModels, function* (viewModel) {
            ...
            yield* _co.coEach(deliveries, function* (delivery) {
                var driver = yield self.driverModel.findOne({ _id: objectId(delivery.driver) });
...
Controller.prototype.getUnconfirmed = function (query) {
    ...
    return model.find(parameters).sort({ "number": 1 }).populate('sender').populate('destination').skip(skip).limit(limit).exec();
};
...
Controller.prototype.getDeliveryList = function (query) {
    ...
    return model.find(parameters).sort({ "number": 1 }).populate('sender destination regions.destination payment.type').skip(skip).limit(limit).exec();
};
