...
Controller.prototype.getAll = function (query) {
    ...
    var parameters = { "inputLocation": objectId(query['location']), "sender": { "$ne": objectId(defaultComponent.client) } };
    ...
    return model.find(parameters).sort({ "number": -1 }).populate('sender destination payment.type items.itemType items.packingType')
                                 .skip(skip)
                                 .limit(limit)
                                 .lean()
                                 .exec();
